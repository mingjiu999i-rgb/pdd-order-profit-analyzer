...
def build_xlsx(conn: sqlite3.Connection, shop_name: str | None = None, start_date: str | None = None, end_date: str | None = None) -> bytes:
    ...
    headers = ["店铺名称", "成交日期", "订单数", "订单成交额", "有效订单销售额", "已发货退款", "其他扣款", "订单净回款", "入账完成率", "净结算率", "入账状态", "说明"]
    _title(ws, f"{shops}｜拼多多订单经营分析", f"数据生成时间：{as_of}　口径：按订单原始支付日期归集，金额单位为人民币元", len(headers))
    ws["A3"] = "说明：订单净回款已剔除推广实际消耗和推广充值；净结算率＝订单净回款÷有效订单销售额。退款和资金数据会持续回填。"
    ...
    for n, d in enumerate(report_rows, 5):
        ...
        values = [shop_label, d["date"], d["order_count"], d["original_sales"], d["effective_sales"], d["shipped_refund"], d["other_deductions"], d["current_net"], d["completion_rate"], d["net_settlement_rate"], status, "；".join(notes)]
        ...
        for c in range(4, 9): ws.cell(n, c).number_format = '¥#,##0.00;[Red]-¥#,##0.00'
        for c in (9, 10): ws.cell(n, c).number_format = "0.00%"
        ws.cell(n, 11).fill = PatternFill("solid", fgColor=GREEN if d["mature"] else YELLOW)
    _finish(ws, [20,13,10,14,17,14,13,15,13,13,13,34], "A5")
